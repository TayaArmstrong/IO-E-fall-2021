...
function draw() {
    ...
    var Posx = map(diameter1, -1, 1, 0, width);
    var Posy = map(diameter0, -1, 1, 0, height);
    if (holdOff == false) {
        ellipse(Posx, Posy, 20, 20);
    }

    var noteX = map(diameter1, -1, 1, -24, 24); //24 steps = 2 octaves
    noteX = noteX.toFixed(0);
    var freq = 440 * 2 ** (noteX / 12);
    console.log(freq, noteX);
    osc1.freq(freq);

    var noteY = map(diameter0, -1, 1, -24, 24); //24 steps = 2 octaves
