...
function gotData() {
    ...
    parame0 = splitter[0]; //put the first sensor's data into a variable
    parame1 = splitter[1];
    parame2 = splitter[2];
    holdOff = false;
...
function draw() {
    ...
    var Posx = map(parame1, -1, 1, 0, width);
    var Posy = map(parame0, -1, 1, 0, height);
    if (holdOff == false) {
        ellipse(Posx, Posy, 7, 7);
    }

    var noteX = map(parame1, -1, 1, -24, 24); //24 steps = 2 octaves
    noteX = noteX.toFixed(0);
    var freq = 440 * 2 ** (noteX / 12);
    console.log(freq, noteX);
    osc1.freq(freq);

    var noteY = map(parame0, -1, 1, -24, 24); //24 steps = 2 octaves
    noteY = noteY.toFixed(0);
    var freq2 = 440 * 2 ** (noteY / 12);
    console.log(freq2, noteY);
    osc2.freq(freq2);
    ...
    //    var freq = map(diameter0, 0, 1, 0, 880);
    //    osc1.freq(freq);
    //    console.log(freq, width);
    //
